...
namespace MergeSort
{
    public class BitonicMergeSort : System.IDisposable
    {
        ...
        public const string KERNEL_SORT_EDGEVERTICES = "BitonicSortEdgeVertices";
        ...
        public const string BUF_EDGEVERTEX_VALUES = "EdgeVertices";

        readonly ComputeShader _compute;
        readonly int _kernelSort, _kernelSortInt, _kernelInit, _kernelSortEdgeVertices;
        ...
        public BitonicMergeSort(ComputeShader compute)
        {
            ...
            _kernelSortEdgeVertices = compute.FindKernel(KERNEL_SORT_EDGEVERTICES);
        }
        ...
        public void SortEdgeVertices(ComputeBuffer keys, ComputeBuffer values)
        {
            ...
            for (var dim = 2; dim <= count; dim <<= 1)
            {
                ...
                for (var block = dim >> 1; block > 0; block >>= 1)
                {
                    ...
                    _compute.SetBuffer(_kernelSortEdgeVertices, BUF_KEYS, keys);
                    _compute.SetBuffer(_kernelSortEdgeVertices, BUF_EDGEVERTEX_VALUES, values);
                    _compute.Dispatch(_kernelSortEdgeVertices, x, y, z);
                }
            }
        }
